...
@php
    $ffpadApps = [
        [
            ...
            'hue' => 'linear-gradient(145deg, #10b981, #059669)',
        ],
        [
            'key' => 'flows',
            ...
            'hue' => 'linear-gradient(145deg, #8b5cf6, #6d28d9)',
        ],
        [
            'key' => 'bulk-editor',
            ...
            'hue' => 'linear-gradient(145deg, #3b82f6, #1d4ed8)',
        ],
        [
            'key' => 'locksmith',
            ...
            'hue' => 'linear-gradient(145deg, #f59e0b, #d97706)',
        ],
        [
            'key' => 'pos',
            ...
            'hue' => 'linear-gradient(145deg, #f43f5e, #be123c)',
        ],
    ];
...
.ffpad-tab{position:fixed;right:0;top:50%;z-index:460;display:flex;flex-direction:column;align-items:center;gap:10px;padding:16px 9px 14px;border:0;border-radius:12px 0 0 12px;cursor:pointer;user-select:none;-webkit-user-select:none;background:linear-gradient(180deg,rgba(255,255,255,.16),rgba(255,255,255,0) 38%),linear-gradient(160deg,#6d5ef0 0%,#5b4fe6 45%,#4c3fd6 100%);box-shadow:inset 1.5px 1.5px 1px rgba(255,255,255,.35),inset -1px -2px 3px rgba(20,10,60,.45),-6px 8px 18px -6px rgba(43,25,148,.5);transform:translateY(-50%) translateZ(0);will-change:transform;transition:transform .2s cubic-bezier(.2,.7,.3,1),opacity .2s ease}
.ffpad-tab::after{content:"";position:absolute;inset:0;border-radius:inherit;box-shadow:-12px 14px 30px -8px rgba(43,25,148,.55);opacity:0;transition:opacity .2s ease;pointer-events:none}
...
.ffpad-tab__grip i{width:3px;height:3px;border-radius:50%;background:rgba(255,255,255,.55)}
.ffpad-tab__label{writing-mode:vertical-rl;text-orientation:mixed;transform:rotate(180deg);color:#fff;font-size:11.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;text-shadow:0 1px 2px rgba(20,10,60,.45);font-family:inherit}
.ffpad-tab__dot{width:7px;height:7px;border-radius:50%;background:radial-gradient(circle at 30% 30%,#7dffc9,#10b981);box-shadow:0 0 6px rgba(16,185,129,.8)}
.ffpad-scrim{position:fixed;inset:0;z-index:470;background:rgba(10,10,18,.42);opacity:0;visibility:hidden;transition:opacity .22s ease,visibility 0s linear .22s}
.ffpad-scrim.is-open{opacity:1;visibility:visible;transition:opacity .22s ease}
.ffpad-drawer{position:fixed;right:0;top:50%;z-index:480;width:min(348px,calc(100vw - 40px));max-height:min(78vh,640px);display:flex;flex-direction:column;border-radius:18px 0 0 18px;overflow:hidden;background:#fbfbfd;box-shadow:-24px 30px 70px -22px rgba(20,10,60,.5),-2px 4px 14px rgba(20,10,60,.18);transform:translateY(-50%) translateX(105%) translateZ(0);opacity:0;visibility:hidden;will-change:transform,opacity;transition:transform .28s cubic-bezier(.2,.7,.3,1),opacity .22s ease,visibility 0s linear .28s;font-family:inherit}
...
.ffpad-drawer__head{display:flex;align-items:center;gap:12px;padding:16px 16px 14px;background:radial-gradient(120% 140% at 0% 0%,rgba(124,108,255,.35),transparent 55%),linear-gradient(160deg,#17122b,#0a0a12)}
.ffpad-drawer__mark{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;color:#fff;font-weight:800;font-size:17px;background:linear-gradient(145deg,#6d5ef0,#4c3fd6);box-shadow:inset 1px 1.5px 1px rgba(255,255,255,.4),inset -1px -2px 3px rgba(20,10,60,.5);flex-shrink:0}
.ffpad-drawer__title{color:#fff;font-size:14.5px;font-weight:700;letter-spacing:.01em;line-height:1.2;margin:0}
.ffpad-drawer__sub{color:rgba(255,255,255,.62);font-size:11.5px;line-height:1.35;margin:2px 0 0}
...
.ffpad-card{position:relative;display:flex;align-items:center;gap:12px;padding:11px 12px;border-radius:14px;text-decoration:none;background:#fff;border:1px solid rgba(20,16,31,.07);box-shadow:0 1px 2px rgba(20,16,31,.05);transition:transform .16s cubic-bezier(.2,.7,.3,1),border-color .16s ease}
.ffpad-card::after{content:"";position:absolute;inset:0;border-radius:inherit;box-shadow:0 14px 28px -14px rgba(91,79,230,.45);opacity:0;transition:opacity .16s ease;pointer-events:none}
.ffpad-card:hover{transform:translateY(-2px) translateZ(0);border-color:rgba(91,79,230,.35)}
.ffpad-card:hover::after{opacity:1}
.ffpad-card:active{transform:translateY(0) scale(.995)}
.ffpad-card__tile{position:relative;width:44px;height:44px;border-radius:12px;overflow:hidden;display:grid;place-items:center;color:#fff;font-weight:800;font-size:18px;flex-shrink:0;box-shadow:0 5px 12px -5px rgba(20,16,31,.45);text-shadow:0 1px 2px rgba(0,0,0,.3)}
.ffpad-card__tile img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}
.ffpad-card__tile::after{content:"";position:absolute;inset:0;border-radius:inherit;pointer-events:none;box-shadow:inset 0 1px 1px rgba(255,255,255,.45),inset 0 -1.5px 3px rgba(0,0,0,.18),inset 0 0 0 1px rgba(20,16,31,.06)}
.ffpad-card__body{min-width:0}
.ffpad-card__name{color:#14101f;font-size:12.8px;font-weight:700;line-height:1.25;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.ffpad-card__tag{color:#6b6880;font-size:11px;line-height:1.35;margin:2px 0 0;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.ffpad-card__meta{display:flex;align-items:center;gap:7px;margin-top:4px;font-size:10.5px;font-weight:600}
.ffpad-card__stars{color:#b58900}
.ffpad-card__stars b{color:#14101f}
.ffpad-card__free{padding:1px 7px;border-radius:999px;background:rgba(16,185,129,.12);color:#0b8a60}
.ffpad-card__go{margin-left:auto;color:#b7b3c9;font-size:15px;flex-shrink:0;transition:transform .16s cubic-bezier(.2,.7,.3,1),color .16s ease}
.ffpad-card:hover .ffpad-card__go{transform:translateX(3px);color:#5b4fe6}
.ffpad-drawer__foot{padding:11px 16px 13px;border-top:1px solid rgba(20,16,31,.07);background:#fff}
.ffpad-drawer__foot a{color:#5b4fe6;font-size:11.5px;font-weight:700;text-decoration:none}
...
<aside id="ffpad-drawer" class="ffpad-drawer" role="dialog" aria-modal="true" aria-label="Airo Apps — partner apps">
    <header class="ffpad-drawer__head">
        <span class="ffpad-drawer__mark" aria-hidden="true">A</span>
        <div>
    ...
    <div class="ffpad-drawer__list">
        @foreach ($ffpadApps as $ffpadApp)
            <a class="ffpad-card" href="{{ $ffpadApp['url'] }}" target="_blank" rel="noopener noreferrer">
                <span class="ffpad-card__tile" style="background: {{ $ffpadApp['hue'] }}" aria-hidden="true">
                    {{ $ffpadApp['monogram'] }}
